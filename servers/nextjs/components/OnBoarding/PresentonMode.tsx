...
const PresentonMode = ({
    ...
    const getProviderSetupTitle = (label: string) =>
        locale === "zh-CN" ? `${t(label)}设置` : `${label} setup`;
        ...
        return (
            <div className="col-span-full rounded-[10px] border border-[#EDEEEF] bg-[#FBFBFD] p-4 shadow-[0_12px_28px_rgba(16,19,35,0.04)]">
                <div className="mb-4 flex items-start justify-between gap-3">
                    <div>
                        <p className="text-sm font-semibold text-[#191919]">{getProviderSetupTitle(provider.label)}</p>
                        <p className="mt-1 text-xs leading-5 text-gray-500">
                            {t("Configure the selected image provider before continuing.")}
                        </p>
                    </div>
                    {provider.getApiKeyUrl && (
                        ...
                        >
                            {t("Get API Key")} <ArrowUpRight className="h-3.5 w-3.5" />
                        </a>
                    )}
                </div>
    ...
    const renderSelectedWebSearchProviderConfig = () => {
        ...
        return (
            <div className="col-span-full rounded-[10px] border border-[#EDEEEF] bg-[#FBFBFD] p-4 shadow-[0_12px_28px_rgba(16,19,35,0.04)]">
                <div className="mb-4">
                    <p className="text-sm font-semibold text-[#191919]">{getProviderSetupTitle(selectedWebProvider.label)}</p>
                    <p className="mt-1 text-xs leading-5 text-gray-500">
                ...
                <div className="space-y-4">
                    {selectedWebProvider.value === "auto" && (
                        <div className="rounded-lg border border-[#D9D6FE] bg-[#F4F3FF] p-3 text-xs leading-5 text-[#5146E5]">
                            {t("Presenton will use model-native web grounding when available. If the selected text model does not support it, web search stays off until you choose an external provider.")}
                        </div>
                    )}
    ...
    return (
        <div className='w-full max-w-[660px] font-syne pb-10'>
            ...
            <div className='flex items-center gap-2 bg-[#F0F3F9B2] rounded-[8px]  px-6 py-2.5 my-[54px]'>
                ...
                <p className='text-sm text-[#5F6062] font-medium'>{t("Runs locally on your device. Your API keys and generation setup stay on your machine.")}</p>
            </div>
                            ...
                            {LOCAL_PROVIDERS.map((value) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-[#191919]">{t(provider.label)}</p>
                            ...
                            {llmConfig.LLM === 'ollama' ? (
                                ...
                                <>
                                    <div className='flex items-center justify-between mb-2'>
                                        ...
                                        {llmConfig.LLM && LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl && <a href={LLM_PROVIDERS[llmConfig.LLM!]?.getApiKeyUrl || ""} target='_blank' className='text-[#666666] text-xs font-normal flex items-center gap-1'>{t("Get API Key")} <ArrowUpRight className='w-3.5 h-3.5' /></a>}
                                    </div>
                                    ...
                                    <React.Fragment key={`image-provider-row-${rowIndex}`}>
                                        {row.map((provider) => (
                                            ...
                                            >
                                                ...
                                                <span className="text-xs font-semibold text-[#191919]">{t(provider.label)}</span>
                                            </button>
                                        ))}
